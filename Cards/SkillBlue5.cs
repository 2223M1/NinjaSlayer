...
public sealed class SkillBlue5 : ModCardTemplate
{
    ...
    public SkillBlue5() : base(energyCost, type, rarity, targetType, shouldShowInCardLibrary) { }

    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        ArgumentNullException.ThrowIfNull(cardPlay.Target);
        DamageFocusPower? power = await PowerCmd.Apply<DamageFocusPower>(choiceContext, Owner.Creature, 1, Owner.Creature, this);
        if (power != null)
        {
            power.FocusTarget = cardPlay.Target;
