...
parser=argparse.ArgumentParser();parser.add_argument('--require-runtime',action='store_true');a=parser.parse_args()
...
if a.require_runtime:
    ...
    checks['staged_dll_diff']=int(sha(P/'.godot/mono/temp/bin/Debug/NinjaSlayer.dll')!=sha(P/'build/select-poster-runtime/isolated-game/mods/NinjaSlayer/NinjaSlayer.dll'))
    checks['staged_pack_diff']=int(sha(P/'build/select-poster-runtime/NinjaSlayer.pck')!=sha(P/'build/select-poster-runtime/isolated-game/mods/NinjaSlayer/NinjaSlayer.pck'))
