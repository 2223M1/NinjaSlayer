...
运行 `preview.ps1 -Assembly <候选DLL> -Pack <候选PCK>`。它使用独立 APPDATA 和游戏文件的只读硬链接；替换测试 DLL/PCK
...
再运行 `python tools/character-select/verify_poster.py --require-runtime --assembly <实测DLL> --pack <实测PCK>`。
安装和发布由用户之后统一进行。
