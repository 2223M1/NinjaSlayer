...
function validateManifest(value) {
  if (value.schemaVersion !== 3) throw new Error('compatibility.json schemaVersion must be 3.');
...
function validateHostContract(name, contract) {
  if (!contract
      || Object.keys(contract).join(',') !== 'moduleMvid'
      || !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(contract.moduleMvid ?? '')) {
    throw new Error(`${name}.hostContract must contain one valid moduleMvid.`);
  }
}
