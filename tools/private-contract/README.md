...
- `-RunnerPurpose Contract`: pass `GameDataDirectoryStable`, `GameDataDirectoryPreview`, `GameResourcePackStable`, and `GameResourcePackPreview`. Native card selection and bilingual formatting require the host PCK. The launcher copies both reference sets and PCKs read-only and creates an isolated .NET 9 runtime. Run from elevated PowerShell 7 (`pwsh`) so the job can block non-loopback traffic from both dotnet and Godot while retaining the local Contract fixture.
