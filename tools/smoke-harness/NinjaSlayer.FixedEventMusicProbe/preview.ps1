...
$root=Join-Path (Split-Path $repo) 'outputs/fixed_event_music/live'
...
Copy-Item -LiteralPath (Join-Path (Split-Path $root) 'compile/stable/Release/NinjaSlayer.dll') -Destination $mod -Force
...
Copy-Item -LiteralPath (Join-Path (Split-Path $root) 'compile/stable/Release/Box2D.NET.dll') -Destination $box -Force
...
try {
    ...
    & ssh.exe localadmin "if (!(Get-NetFirewallRule -DisplayName 'NinjaSlayer-FixedEventMusicQA' -ErrorAction SilentlyContinue)) { New-NetFirewallRule -DisplayName 'NinjaSlayer-FixedEventMusicQA' -Direction Outbound -Action Block -Program '$exe' | Out-Null }"
