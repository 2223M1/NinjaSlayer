...
The job runs only on an ephemeral self-hosted Windows runner labeled `ninjaslayer-contract`. From an elevated PowerShell session, use `tools/private-contract/Start-EphemeralContractRunner.ps1` with the short-lived registration token, exact runner version, and archive SHA-256 shown by GitHub's **New self-hosted runner** page. The optional `RunnerArchivePath` supports a pre-downloaded official archive when elevated network access is unreliable; its SHA-256 is still mandatory. The launcher creates an isolated read-only copy of `sts2.dll`, `0Harmony.dll`, and `GodotSharp.dll`; no private binary is uploaded to GitHub or placed in a cache.
