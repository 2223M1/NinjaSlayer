...
$expectedProperties = @(
    'candidateSha', 'channel', 'compatibilityManifestSha256', 'gameApiVersion',
    'gameAssemblyVersion', 'gameModuleMvid', 'mode', 'repository', 'result',
    'ritsuLibPackageId', 'ritsuLibVersion', 'schemaVersion', 'verifiedContracts',
    'workflowRunId'
) | Sort-Object
...
foreach ($artifact in $artifacts) {
    ...
    try {
        ...
        if ($attestationFiles.Count -ne $channelNames.Count) {
            throw "Artifact must contain exactly $($channelNames.Count) Contract attestations."
        }
        ...
        foreach ($channelName in $channelNames) {
            ...
            if (Compare-Object @($attestation.PSObject.Properties.Name | Sort-Object) $expectedProperties) {
                throw "$channelName Contract attestation contains missing or unexpected fields."
            }
            Assert-NinjaSlayerEqual ([int]$attestation.schemaVersion) 4 "$channelName.schemaVersion"
            ...
            if ($verifiedContracts.Count -ne 1 -or $verifiedContracts[0] -ne 'prepared-lifecycle-publishers') {
                throw "$channelName.verifiedContracts does not prove prepared-lifecycle-publishers."
            }
            ...
            Copy-Item -LiteralPath $attestationPath -Destination (Join-Path $verifiedHostDirectory 'attestation.json') -Force
        }
        Write-Output "Verified protected Contract run $($run.id) for $candidate on stable and preview."
        return
    }
    catch {
...
throw "No matching Contract artifact passed provenance validation. $($failures -join ' | ')"
