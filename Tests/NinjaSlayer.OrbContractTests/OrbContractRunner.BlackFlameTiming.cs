...
public partial class OrbContractRunner
{
    ...
    private static async Task VerifyBlackFlameTiming()
    {
        ...
        foreach (int held in new[] { 0, 1, 2 })
        {
            ...
            int burnDamage = held == 0 ? 0 : held * 4 + 3;
            Require(combat.Enemy.CurrentHp == 1000 - (upgraded ? 33 : 27) - burnDamage
                && BurnSounds.Count == (held == 0 ? 0 : 1),
                "BS1260 must burn only pre-existing held flames, with one sound for their merged wave.");
            ...
            Require(hits.Length == 1 && hits[0].Result.TotalDamage == (held + 1) * 4 + 3 && BurnSounds.Count == 1,
                "The next attack must include the newly generated flame and play one native burn sound.");
        }
        using (var combat = new OrbCombat())
        {
            ...
            Require(burns.SequenceEqual(new[] { 4, 8 }) && BurnSounds.Count == 2,
                "Repeated BS1260 plays must capture a new hand each time: no burn, then four, then eight.");
        }
        using (var combat = new OrbCombat())
        {
            BurnSounds.Clear();
