...
[2026年05月27日]()[964 字]()[大概 4 分钟]()[alkaid616]()

可以使用 RitsuLib 提供的 `ModCustomReward` 基类来极简地实现自定义奖励。

`ModCustomReward` 在原版基类的基础上进行了封装，它可以帮你：

- 自动处理图标UI：不用手写繁琐的 Godot 节点层级，只需提供一个 `res://` 图标资源路径。
- 自动读取本地化文本：轻松设置语言表与字符 Key。
- 辅助存读档：封装了 `IModSerializableReward` 的系列接口，方便你注入自己的持久化 Payload（比如保存每次随机生成的不固定金币数）。
...
- 但是！如果你在 `OnSelect()` 里面包含随机数检定或者获取仅本地相关的资源，不同客户端可能会因为结果不一致导致断连。

所以确保你的 `OnSelect()` 中执行的逻辑：

- 严格确定：利用原版 `RunState.Rng` 的随机序列，或者所有计算因子两边完全对等。
- 基于原版同步：直接派发类似于 `PlayerCmd.GainGold` 的这种已被开发组封装好的带有完整状态分发的网络指令。
