...
<!-- Source: https://tutorials.sts2modding.com/docs/04-ritsulib/04-22-3-in-run-data/ -->

`RitsuLib` 提供了 `RunSavedData` 和 `PlayerRunSavedData` 来帮助你实现局内数据持久化，同时还自带了开局大厅暂存和多人联机同步的支持。
...
这部分是在跑局刚开始建立前用到的。许多时候，我们需要玩家在大厅界面（比如选人、选挑战选项时）更改自己想要的局内数据。
...
```
前面注册槽位时，我们传入了 `SyncLobbyOnChange = true`。这意味着只要你在这里调用了 `Lobby.Modify`，RitsuLib 就会自动帮你把这个数据的改动同步给主机和队友。
...
主机合并了本地或远端玩家贡献。
`PlayerJoined`
新玩家进入大厅，RitsuLib 给 session 补玩家槽。
`Manual`
你调用了 `RunSavedDataLobby.NotifyStagingChanged(lobby)`。
`Committing`
主机即将构建新开局快照。
...
适合场景
`WhenSet`
默认选择。只有通过 `Set` 或 `Modify` 显式改过的值才写入。
`WhenNonDefault`
默认对象可以被反复读取，但只有和默认值不同才进存档。适合挑战开关、计数器。
`AlwaysWhenRegistered`
只要槽位能解析就写入。适合每局都必须带 schema 的控制数据。
