...
<!-- Source: https://tutorials.sts2modding.com/docs/08-launch-arguments/ -->
...
指定本地测试玩家ID。
`+connect_lobby`
`+connect_lobby 12345678901234567`
启动后按Steam大厅ID自动加入。
`nomods`
`--nomods`
不启用mod模式。
`force-steam`
`--force-steam` 或 `force-steam=on` / `force-steam=off`
强制开启或关闭Steam初始化。
`-log`
`-log Net Info`
设置指定日志类型的输出级别。
...
`-wpos 100 200`
窗口模式下放置窗口的位置。
Godot自带命令行参数： [https://docs.godotengine.org/zh-cn/4.x/tutorials/editor/command_line_tutorial.html](https://docs.godotengine.org/zh-cn/4.x/tutorials/editor/command_line_tutorial.html)
