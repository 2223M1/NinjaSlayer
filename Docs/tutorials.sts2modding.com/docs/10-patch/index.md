...
按位置对应第 0、1… 个参数，原名不方便写或想统一处理多个方法时可用。
...
注入当前所挂原方法的 `MethodBase`；不能用来调用原方法。
...
[https://harmony.pardeike.net/articles/utilities.html](https://harmony.pardeike.net/articles/utilities.html)
...
当目标方法不好用特性写死（嵌套类、需要按名字筛选、一次打多个方法）时，在补丁类里写这些辅助方法。类上仍要有 `[HarmonyPatch]`，`PatchAll` 才会扫描到。
...
`[HarmonyPriority(int)]`：数值越大，越先执行（默认 `Priority.Normal` = 400）。
...
使用`[HarmonyBefore(string[])]` / `[HarmonyAfter(string[])]`相对其它 Harmony 实例的 id 排序（即 `new Harmony("这个字符串")` 里的 id），而不是 int 优先级。
