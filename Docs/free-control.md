...
Contact speed includes angular motion around the center of mass. Below 2500 px/s it deals no collision damage, keeping running, jumping, dashing and fast falling harmless on contact with a stationary enemy. From 2500 to 10000 px/s, damage rises quadratically from 1 to a maximum of 10: `1 + floor(9 * clamp((speed - 2500) / 7500, 0, 1)^2)`. Full damage therefore requires five times the previous 2000 px/s speed. A target can receive physical/Tornado contact damage at most once every 0.15 seconds. A simultaneous fixed strike and physical impact use the larger value. Card animation travel is not included in physical velocity. Shuriken use the first enemy crossed by their straight path and can miss.
